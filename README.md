...
- Kids I Spy alternates roles: Reachy turns its rotating base and head through a bounded three-frame search when choosing, then the child chooses a safe household object and gives clues while Reachy makes up to six guesses with small base-and-head thinking turns and no camera. Camera access is revoked before guessing, targets use a strict broker schema, and bridge state is deleted on Stop/expiry.

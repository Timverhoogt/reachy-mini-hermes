...
The simpler [Reachy Mini I Spy](https://github.com/Timverhoogt/reachy-mini-i-spy) project is the canonical standalone English/Dutch game for installations that do not need the full Hermes Reachy application. It uses its own three-frame choreography and dedicated six-route broker. The projects share compatible target-selection principles but deliberately retain separate camera, moderation, cancellation, motion, and speech authority; coordinated policy changes must follow the standalone [safety contract](https://github.com/Timverhoogt/reachy-mini-i-spy/blob/main/docs/SAFETY_CONTRACT.md) and both projects' regression suites.

...
Realtime and browser robot controls are allow-listed to local head direction, recorded emotion, and recorded dance actions. The UI and model cannot submit raw joints, arbitrary move names, shell commands, Home Assistant, files, or arbitrary Hermes tools. Browser actions pass through the same bounded queue, movement arbitration, power-mode checks, and cancellation generation as Realtime actions; they are rejected in Meeting/Sleep and automatically run the native wake transition when invoked from Standby. **Stop movement** cancels active and queued moves and restores media playback if SDK cancellation stopped it. Agent capabilities remain behind the authenticated `ask_hermes` boundary.

...
Realtime and browser robot controls are allow-listed to local head direction, recorded emotion, and recorded dance actions. The UI and model cannot submit raw joints, arbitrary move names, shell commands, Home Assistant, files, or arbitrary Hermes tools. Browser actions pass through the same bounded worker, movement arbitration, power-mode checks, and cancellation generation as Realtime actions; manual requests reject additional queueing while busy, and the worker rechecks privacy immediately before physical execution. They are rejected in Meeting/Sleep and automatically complete the native wake transition when invoked from Standby. **Stop movement** is an out-of-band priority action: it cooperatively cancels active and queued moves without changing power mode, initiating a pose, or stopping the voice playback pipeline. Agent capabilities remain behind the authenticated `ask_hermes` boundary.
